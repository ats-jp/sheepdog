 ...
 * テーブル名 t_member
 */
public class t_memberDAO extends GenericDAO<t_memberDTO> {

	/**
	 * パラメータの条件にマッチするレコードを検索し、 {@link t_memberIterator} として返します。
	 * <br>
	 ...
	 */
	public t_memberIterator select(
		Condition condition,
		OrderByClause order,
	 ...
	 * @return {@link DTOIterator}
	 */
	public t_memberIterator select(Condition condition, OrderByClause order) {
	...
	public t_memberIterator select(
		...
		SQLAdjuster adjuster,
		RowLockOption rowLockOption) {
		return new t_memberIterator(helper.getUpdatableDataObjects(
			optimizer,
			condition,
			order,
			adjuster,
	...
	/**
	 * t_memberDTO を生成するメソッドです。
	 */
	@Override
	protected t_memberDTO createDTO(UpdatableDataObject data) {
		return new t_memberDTO(data);
	}

	@Override
	protected ResourceLocator getResourceLocator() {
		return t_member.RESOURCE_LOCATOR;
	}

	/**
	 * {@link t_memberDAO} が使用する Iterator クラスです。
	 */
	public class t_memberIterator extends DTOIterator<t_memberDTO> {
		 ...
		 * @param iterator 
		 */
		private t_memberIterator(
			DataObjectIterator<UpdatableDataObject> iterator) {
			super(iterator);
		}

		@Override
		public t_memberDTO next() {
