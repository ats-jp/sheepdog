 ...
 * テーブル名 t_content
 */
public class t_contentDAO extends GenericDAO<t_contentDTO> {

	/**
	 * パラメータの条件にマッチするレコードを検索し、 {@link t_contentIterator} として返します。
	 * <br>
	 ...
	 */
	public t_contentIterator select(
		Condition condition,
		OrderByClause order,
	 ...
	 * @return {@link DTOIterator}
	 */
	public t_contentIterator select(Condition condition, OrderByClause order) {
	...
	public t_contentIterator select(
		...
		SQLAdjuster adjuster,
		RowLockOption rowLockOption) {
		return new t_contentIterator(helper.getUpdatableDataObjects(
			optimizer,
			condition,
			order,
			adjuster,
	...
	/**
	 * t_contentDTO を生成するメソッドです。
	 */
	@Override
	protected t_contentDTO createDTO(UpdatableDataObject data) {
		return new t_contentDTO(data);
	}

	@Override
	protected ResourceLocator getResourceLocator() {
		return t_content.RESOURCE_LOCATOR;
	}

	/**
	 * {@link t_contentDAO} が使用する Iterator クラスです。
	 */
	public class t_contentIterator extends DTOIterator<t_contentDTO> {
		 ...
		 * @param iterator 
		 */
		private t_contentIterator(
			DataObjectIterator<UpdatableDataObject> iterator) {
			super(iterator);
		}

		@Override
		public t_contentDTO next() {
