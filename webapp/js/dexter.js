var Dexter = Class.create({
	...
	adjustAllValidationMessages: function() {
		for (var key in this.messageAreas) {
			...
			this.adjustPosition(value["input"], value["message"]);
		}
	},
	...
	drawValidationMessage: function(element, validationSuccess, validationMessage) {
		...
		if (!messageValues) {
			...
			this.messageAreas[element.id] = {input: element, message: messageArea, validationSuccess: validationSuccess};
		...
		if (!validationSuccess || (validationMessage != null && validationMessage.length > 0)) {
			...
			this.adjustPosition(element, messageArea);
	...
	showDescription: function(x, y, validatorNodes, element) {
		...
		//フォーム要素にかかった時の調整
		var bottomPosition = element.offsetHeight + element.viewportOffset().top;
